...
trait Tag {
  ...
  object HH {
    implicit def hh: Tagg[HH] {type Out = String} = new Tagg[HH] { type Out = String}
  }
  type HH = HH.type
  ...
  object TT {
    trait Tagg[A] {
      type Out
      def build(a: Out): II[A] = new II[A] { def value(implicit ev: Tagg[A]): ev.Out = a.asInstanceOf[ev.Out]}
    }

    object Tagg { type Aux[I,O] = Tagg[I] { type Out = O }
      case class Curry[A] (){
    ...
    object HH {
      implicit def hh: Tagg.Aux[HH,String] = new Tagg[HH] { type Out = String}
    }
    type HH = HH.type
